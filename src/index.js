...
const createTextSelectionInterceptor = require("./createTextSelectionInterceptor.js");
...
const makeSvgController = require("./svgController.js");
const makeDomController = require("./domController.js");
...
function createPanZoom(domElement, options) {
  ...
  const beforeSingleFingerTouch = options.beforeSingleFingerTouch || noop;
  const beforeMultiFingerTouch = options.beforeMultiFingerTouch || noop;
  const beforeDoubleClick = options.beforeDoubleClick || noop;
  let speed =
  ...
  function zoomByRatio(clientX, clientY, ratio, skipEventTrigger = false) {
  ...
  function zoomAbs(clientX, clientY, zoomLevel, skipEventTrigger = false) {
    const ratio = zoomLevel / transform.scale;
    zoomByRatio(clientX, clientY, ratio, skipEventTrigger);
  }
  ...
  function onTouch(e) {
    ...
    if (e.touches.length === 1) {
      if (beforeSingleFingerTouch(e)) return;
      return handleSingleFingerTouch(e, e.touches[0]);
    }
    if (e.touches.length === 2) {
      if (beforeMultiFingerTouch(e)) return;
  ...
  function publicZoomTo(
    ...
    smoothScroll.cancel();
    cancelZoomAnimation();
    return zoomByRatio(clientX, clientY, scaleMultiplier, skipEventTrigger);
  }
...
function autoRun() {
  ...
  function getPanzoomAttributeNameValue(attr) {
    ...
    if (!isPanZoomAttribute) return null;
